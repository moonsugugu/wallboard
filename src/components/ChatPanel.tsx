...
export default function ChatPanel({ messages, defaultAuthor, onClose, onSend }: Props) {
  ...
  return (
    <div style={{ boxShadow: 'var(--shadow-lift)' }}
      className="fixed bottom-24 right-6 z-40 flex h-[28rem] w-80 max-w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-[26px] border border-[var(--color-border)] bg-[var(--color-surface)]">
      <div className="flex items-center justify-between border-b border-[var(--color-border)] px-4 py-3">
        <h3 className="font-display text-[19px] text-[var(--color-ink)]">실시간 채팅</h3>
        <button type="button" onClick={onClose} className="btn-icon bg-[#2f2a25]/8 text-[var(--color-ink)] hover:bg-[#2f2a25]/20" aria-label="채팅 닫기">
          ×
        </button>
      </div>
      ...
      <div ref={listRef} className="flex-1 space-y-2 overflow-y-auto px-4 py-3">
        ...
        {messages.map((m) => (
          <div key={m.id} className="text-sm">
            <b className="font-semibold text-[var(--color-ink)]">{m.author || '익명'}</b>{' '}
