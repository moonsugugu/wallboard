...
import { addPost, isMine, rememberMine, removePost, subscribeBoard, subscribePosts } from '../lib/db'
import { normalizeBoardCode } from '../lib/roomCode'
import type { Board as BoardDoc, Post, PostColor } from '../types'
...
export default function Board() {
  ...
  const [modalColumn, setModalColumn] = useState<string | null | undefined>(undefined)
  ...
  async function handleAddPost(input: { author: string; text: string; imageUrl?: string; color: PostColor; column: string | null }) {
  ...
  return (
    <div className="mx-auto min-h-full max-w-6xl px-4 py-6">
      ...
      {board.layout === 'wall' ? (
        <WallLayout posts={posts} isMine={(id) => isMine(boardCode, id)} onDelete={handleDelete} />
      ) : (
        <ShelfLayout
          ...
          onAddTo={(col) => setModalColumn(col)}
        />
      )}
      ...
      {modalColumn !== undefined && (
        <AddPostModal
          columns={board.columns}
          defaultColumn={modalColumn}
          onClose={() => setModalColumn(undefined)}
